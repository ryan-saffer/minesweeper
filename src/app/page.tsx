...
function Level({
  ...
  return (
    <Link
      className="h-20 p-8 md:p-12 mt-4 flex flex-col justify-center items-center bg-[#BDBDBD] border border-[#848484] cursor-pointer border-t-white border-t-4 border-l-white border-l-4 border-b-[#7B7B7B] border-b-4 border-r-[#7B7B7B] border-r-4"
      href={`/${name.toLowerCase()}`}
    >
      <h3 className="font-serif text-2xl md:text-4xl">{name}</h3>
      <p className="font-serif text-sm md:text-lg">{`${size}x${size} - ${mines} mines`}</p>
    </Link>
  )
}
